...
public class Search {
	...
	public String IterativeDeepeningTreeSearch() {
		...
		String output = null;
        while (true) {
			output = TreeSearchDepthLimited(new FrontierLIFO(), limit);
			if(output != null){
				return output;
			}
			limit++;
	...
	public String IterativeDeepeningGraphSearch() {
		...
		String output;
		while(true){
			output = GraphSearchDepthLimited(new FrontierFIFO(), limit);
			if(output!=null){
				return output;
			}
			limit++;
	...
	private String GraphSearchDepthLimited(Frontier frontier, int limit) {
		...
		if(!explored.contains(node.state) && limit > node.depth){
			explored.add(node.state);
			frontier.insertAll(Expand(node,problem));
			cnt++;
		}
